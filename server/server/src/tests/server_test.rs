...
fn server_uses_frequency_as_tickrate() {
    ...
    let server = Server::new(&config_on_port(port, 250), MockHandler).expect("server should bind");
...
fn server_binds_to_configured_port() {
    let port = reserve_port();
    let server = Server::new(&config_on_port(port, 100), MockHandler).expect("server should bind");
...
fn server_exam_configuration_tickrate() {
    ...
    let server = Server::new(&config, MockHandler).expect("server should bind");
...
fn server_fails_when_port_is_already_in_use() {
    ...
    let result = Server::new(&config_on_port(port, 100), MockHandler);
    drop(holder);
