...
N_AI="${1:-1}"
if [ "$N_AI" -gt "$CLIENTS" ]; then
    CLIENTS="$N_AI"
fi

if [ ! -x "$SERVER" ]; then
    echo "error: server not found or not executable: $SERVER" >&2
    exit 1
fi
...
export ZAPPY_TEAM_SIZE="${ZAPPY_TEAM_SIZE:-6}"
...
if [ "$HOST" = "localhost" ] || [ "$HOST" = "127.0.0.1" ]; then
    ...
    echo "[play] skipping local server start (connecting to remote host $HOST:$PORT)"
    SERVER_PID=""
fi
